  ...
  <card class="flex flex-col h-auto">
    <div class="px-3 py-3">
      <div class="mb-4 border-b border-50 pb-2">
        <h1
          class="text-center mb-2 text-sm font-semibold uppercase tracking-wide"
        >{{__('Import')}} {{card.resourceLabel}}</h1>
        <p class="leading-normal text-center" v-if="card.sample">
          <a
            target="_blank"
      ...
      <form @submit.prevent="processImport" ref="form">
        ...
        <div class="flex">
          <div v-if="errors">
            <p class="text-danger mb-1" :key="index" v-for="(error, index) in errors">{{error[0]}}</p>
          </div>
          <button
            :disabled="working"
            type="submit"
...
export default {
  ...
  methods: {
    ...
    processImport() {
      ...
      Nova.request()
        .post("/nova-vendor/import-card/import/" + this.card.resource, formData)
        .then(({ data }) => {
