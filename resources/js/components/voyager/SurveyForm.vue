<template>
  <div>
    <form method="post" class="form" @submit.prevent="save">
      <section>
        <div
          v-for="(question, index) in dbform.form"
          :id="question.id"
          :key="question.id"
        ...
        >
          <div class="container" v-if="question.id != 'submit'">
            <div class="form-group" :class="{'has-error': form.errors.has(question.id)}">
              ...
              <span
                class="mt-2 rounded border border-red block bg-red-lightest px-4 py-2 text-red-darkest"
                v-show="form.errors.has(question.id)"
              >{{ form.errors.get(question.id) }}</span>
              <a
                :href="nextId(index)"
                class="btn btn-mint smooth"
      ...
      <section :id="finishId" class="section">
        <div class="container">
          ...
          <div class="alert alert-danger" v-show="form.errors.hasErrors()">
            ...
            <ul>
              <li v-for="(error, key) in form.errors.all()">
