<div class="card">
    <div class="card-top text-white" style="background-image: linear-gradient(rgba(0, 0, 0, 0.55),rgba(0, 0, 0, 0.55)), url({{ Storage::url($order->event->image) }})">
        @if(isset($order->event->logo_light))
        <img src="{{ Storage::url($order->event->logo_light) }}" alt="{{ $order->event->title }} Logo" style="width: 75%; margin-bottom: .5em;">
        @else
    ...
    <div class="list-group list-group-flush">
        ...
        @else
        <a id="pay" class="list-group-item list-group-item-action" data-toggle="collapse" href="#collapseExample">
...
@if(!$order->isPaid())
<a href="#" class="list-group-item list-group-item-action rounded bg-red-lightest text-red-darkest hover:bg-red-lighter hover:text-red-darkest mt-6" onclick="event.preventDefault(); document.getElementById('delete-order-form').submit();">
