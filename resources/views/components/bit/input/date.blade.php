...
<div x-data="{ value: @entangle($attributes->wire('model')), picker: undefined }"
    ...
    class="flex mt-1 shadow-sm {{ $withTime ? 'rounded-l-md' : 'rounded-md' }}">
    ...
    <input x-ref="input" x-bind:value="value" autocomplete="off" class="flex-1 block w-full border-gray-300 rounded-none dark:bg-gray-800 dark:text-gray-200 dark:border-gray-600 focus:ring-green-500 focus:border-green-500 rounded-r-md sm:text-sm" {{ $attributes->whereDoesntStartWith('wire:model') }}>
</div>
