    ...
    <body class="bg-gray-100 font-sans antialiased dark:bg-gray-800">
        ...
        >
            ...
            <div class="flex w-0 flex-1 flex-col overflow-hidden">
                ...
                >
                    <div class="py-6">
                        <div class="mx-auto flex max-w-7xl items-center justify-between space-x-1 px-4 sm:px-6 md:px-8">
                            <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-200">{{ $title }}</h1>
                            @isset($action)
                                <x-bit.button.round.primary wire:click="{{ $action['method'] }}">
                                    {{ $action['label'] }}
                                </x-bit.button.round.primary>
                            @endif
                        </div>
                        <div class="mx-auto mt-8 max-w-7xl px-4 sm:px-6 md:px-8">
                            {{ $slot }}
                        </div>
                    </div>
                </main>
