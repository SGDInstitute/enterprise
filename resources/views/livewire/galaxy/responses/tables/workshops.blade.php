            ...
            <x-bit.table.row wire:key="row-{{ $response->id }}">
                <x-bit.table.cell class="pl-6 pr-2">
                    ...
                    @if (isset($assignedWorkshops[$response->id]))
                        <x-bit.button.link
                            size="py-1 px-2"
                            wire:click="editItem({{ $assignedWorkshops[$response->id] }})"
                        >
                            <x-heroicon-o-pencil class="h-4 w-4 text-green-500 dark:text-green-400" />
                        </x-bit.button.link>
                    @elseif (in_array($response->status, ['approved', 'confirmed', 'finalized']) && $event->items->count() > 0)
                        <x-bit.button.link size="py-1 px-2" wire:click="assignTime({{ $response->id }})">
                            <x-heroicon-o-calendar class="h-4 w-4 text-green-500 dark:text-green-400" />
                        </x-bit.button.link>
                    @endif
                </x-bit.table.cell>
                ...
                @if ($form->review)
                    ...
                    <x-bit.table.cell class="px-2">
                        {!! $response->reviews->count() > 0 ? $response->score : '-' !!}
                    </x-bit.table.cell>
                @endif
                ...
                <x-bit.table.cell class="pl-2 pr-6">
                    <x-bit.button.link
                        size="py-1 px-2"
                        href="{{ route('galaxy.responses.show', ['response' => $response]) }}"
                    >
