<div>
    <div x-data="{ show: false }">
        ...
        <div x-show="show" x-cloak x-transition:enter="transition ease-out duration-100 transform" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-75 transform" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95">
            <form wire:submit="sendNotifications" class="flex items-end py-4 space-x-4">
                ...
                <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                    ...
                    <x-bit.input.group :for="$item['id']" :label="$item['question']" :error="$errors->first('advanced.' . $item['id'])">
                        <div class="mt-1 space-y-1">
                            ...
                            @if (strpos($label, ':'))
                            <x-bit.input.checkbox :value="explode(':', $label)[0]" :id="$item['id'].'-'.$key" :label="explode(':', $label)[0]" wire:model.live="advanced.{{ $item['id'] }}" />
                            @else
                            <x-bit.input.checkbox :value="$label" :id="$item['id'].'-'.$key" :label="$label" wire:model.live="advanced.{{ $item['id'] }}" />
                            @endif
                            @endforeach
                            @if (isset($item['list-other']) && $item['list-other'] === true)
                            <x-bit.input.checkbox value="other" label="Other" wire:model.live="advanced.{{ $item['id'] }}" />
                            @endif
                        </div>
                    </x-bit.input.group>
                    @endif
                ...
                <div class="space-y-2">
                    <div class="grid grid-cols-1 gap-2 md:grid-cols-4">
                        <x-bit.input.group for="editing-item-schedule-slot" label="Schedule Slot" class="md:col-span-3">
                            <x-bit.input.select class="w-full mt-1" wire:model.live="editingItem.parent_id" id="editing-item-schedule-slot">
                    ...
                    <x-bit.input.group for="editing-item-track" label="Tracks">
                        <x-bit.input.text class="w-full mt-1" wire:model.live="editingTracks" id="editing-item-track" />
                        <x-bit.input.help>Can be separated by comma</x-bit.input.help>
                    </x-bit.input.group>
                    <x-bit.input.group for="editing-item-speaker" label="Speaker">
                        <x-bit.input.text rows="8" class="w-full mt-1" wire:model.live="editingItem.speaker" id="editing-item-speaker" />
                    </x-bit.input.group>
