<div>
    <div class="grid grid-cols-1 gap-8 md:grid-cols-4">
        ...
        @if ($numberOfResponses > $numberOfUniqueResponses)
            <x-bit.stat
                title="Total - Likely Duplicates"
                subtitle="only unique answers are shown"
                :value="$numberOfUniqueResponses"
            />
        @endif
            ...
            <div class="space-y-4">
                ...
                @else
                    <x-bit.table class="max-h-96">
                        <x-slot name="head">
                            <x-bit.table.heading colspan="2">
                                Answers (obvious non-answers have been filtered out)
                            </x-bit.table.heading>
                        </x-slot>
                        <x-slot name="body">
                            @foreach ($qa['answers'] as $answer)
                                <x-bit.table.row>
                                    ...
                                    <x-bit.table.cell>
                                        <button
                                            type="button"
                                            wire:click="showFullResponse('{{ Str::limit($answer, 20, '') }}')"
                                        >
                                            <x-heroicon-o-eye class="h-4 w-4 text-green-500" />
                                        </button>
                                    </x-bit.table.cell>
                                </x-bit.table.row>
                            @endforeach
    ...
    @if ($foundAnswer)
        <x-bit.modal.dialog wire:model.live="showModal" max-width="6xl">
            <x-slot name="title">
                ...
                <span class="block max-w-prose truncate text-xs">{{ $foundAnswer }}</span>
            </x-slot>
            ...
            <x-slot name="content">
                <div class="space-y-8">
                    <div>
                        ...
                        <div class="flex space-x-4 divide-x border-gray-400">
                    ...
                    @foreach ($survey->form as $question)
                        <div>
                            <h2 class="mb-4 text-lg font-bold text-gray-500 dark:text-gray-300">
                                {{ $question['question'] }}
                            </h2>
                    ...
                    @if ($foundResponses->count() > 1)
                        <div>
                            <div class="flex space-x-4 divide-x border-gray-400">
                                @foreach ($foundResponses as $response)
                                    <div class="flex-1 text-gray-900 dark:text-gray-200">
                                        <x-bit.button.round.secondary wire:click="delete({{ $response->id }})">
                                            Delete
