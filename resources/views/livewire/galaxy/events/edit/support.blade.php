    ...
    <div class="space-y-4">
        ...
        <div wire:key="$index" class="p-4 space-y-2 bg-gray-100 rounded dark:bg-gray-700">
            <x-bit.input.group :for="'faq-name-'.$index" label="Question">
                <x-bit.input.text :id="'faq-name-'.$index" class="block w-64 mt-1" type="text" name="name" wire:model.live="faq.{{ $index }}.name" />
            </x-bit.input.group>
            <x-bit.input.group :for="'faq-content-'.$index" label="Answer">
                <x-bit.input.markdown :id="'faq-content-'.$index" class="block w-full mt-1" type="text" name="content" wire:model.live="faq.{{ $index }}.content" />
            </x-bit.input.group>
        </div>
        @endforeach
    </div>
    <div class="space-y-4">
        ...
        <div class="p-4 space-y-2 bg-gray-100 rounded dark:bg-gray-700">
            <x-bit.input.group for="contact" label="Contact Page">
                <x-bit.input.markdown id="contact" class="block w-full mt-1" type="text" name="content" wire:model.live="contact" />
            </x-bit.input.group>
        </div>
    </div>
</div>
