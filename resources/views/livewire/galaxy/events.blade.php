        ...
        <div class="md:flex md:justify-between">
            ...
            <div class="mt-4 flex items-end space-x-4 md:mt-0">
                ...
                <x-bit.button.round.secondary
                    :href="route('galaxy.events.create')"
            ...
            <x-slot name="body">
                @forelse ($events as $event)
                    <x-bit.table.row wire:key="row-{{ $event->id }}">
                        ...
                        <x-bit.table.cell>
                            {{ $event->formattedStart }} {{ $event->formattedTimezone }}
                        </x-bit.table.cell>
                        <x-bit.table.cell>
                            {{ $event->formattedEnd }} {{ $event->formattedTimezone }}
                        </x-bit.table.cell>
                        <x-bit.table.cell>{{ $event->location }}</x-bit.table.cell>
                        <x-bit.table.cell class="flex space-x-1">
                            <x-bit.button.link
                                size="py-1 px-2"
                                href="{{ route('galaxy.events.show', ['event' => $event ]) }}"
                            >
                                <x-heroicon-o-eye class="h-4 w-4 text-green-500 dark:text-green-400" />
                            </x-bit.button.link>
                            <x-bit.button.link
                                size="py-1 px-2"
                                href="{{ route('galaxy.events.edit', ['event' => $event ]) }}"
                            >
