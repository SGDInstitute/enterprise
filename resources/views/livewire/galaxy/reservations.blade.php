<div>
    <div class="mt-5 flex-col space-y-4">
        <div class="md:flex md:justify-between">
            ...
            <div class="mt-4 flex items-end space-x-2 md:mt-0">
                <x-bit.data-table.per-page />
                <x-bit.button.round.secondary wire:click="comp">Comp</x-bit.button.round.secondary>
                <x-bit.button.round.secondary wire:click="$toggle('showInvoiceModal')">
                    Mark as Paid
            ...
            <x-slot name="head">
                ...
                @if ($this->user === null)
                    <x-bit.table.heading
                        ...
                        :direction="$sortField === 'users.name' ? $sortDirection : null"
                    >
                        Creator
                    </x-bit.table.heading>
                @endif
                ...
                @if ($this->event === null)
                    <x-bit.table.heading
                        sortable
                        wire:click="sortBy('events.name')"
                        :direction="$sortField === 'events.name' ? $sortDirection : null"
                    >
                        Event
                    </x-bit.table.heading>
                @endif
                ...
                <x-bit.table.heading
                    sortable
                    title="Has Invoice"
                    wire:click="sortBy('invoice')"
                    :direction="$sortField === 'invoice' ? $sortDirection : null"
                >
                ...
                <x-bit.table.heading
                    sortable
                    wire:click="sortBy('reservation_ends')"
                    :direction="$sortField === 'reservation_ends' ? $sortDirection : null"
                >
                    Due Date
                </x-bit.table.heading>
                <x-bit.table.heading />
            ...
            <x-slot name="body">
                            ...
                            @unless ($selectAll)
                                <div>
                                    <span>
                                        ...
                                        orders, do you want to select all
                                        <strong>{{ number_format($reservation->total()) }}</strong>
                                        ?
                                    </span>
                                    <x-bit.button.link wire:click="selectAll" class="ml-1 text-blue-600">
                                        Select All
                                    </x-bit.button.link>
                                </div>
                            @else
                ...
                @forelse ($reservations as $reservation)
                    <x-bit.table.row wire:key="row-{{ $reservation->id }}">
                        ...
                        @if ($this->user === null)
                            <x-bit.table.cell>
                                <a href="{{ route('galaxy.users.show', $reservation->user) }}" class="hover:underline">
                        ...
                        @if ($this->event === null)
                            <x-bit.table.cell>
                                <a
                                    href="{{ route('galaxy.events.show', $reservation->event) }}"
                        ...
                        <x-bit.table.cell>
                            {{ $reservation->reservation_ends?->format('M, d Y') }}
                        </x-bit.table.cell>

                        <x-bit.table.cell>
                            <x-bit.button.link
                                size="py-1 px-2"
                                href="{{ route('galaxy.reservations.show', $reservation) }}"
                            >
    ...
    <form wire:submit="markAsPaid">
        <x-bit.modal.dialog wire:model="showInvoiceModal">
                        ...
                        <div wire:key="{{ $id }}">
                            ...
                            <div class="grid grid-cols-2 gap-8">
                                <x-bit.input.group :for="'check-number-' . $id" label="Check Number">
                                    <x-bit.input.text
                                        class="mt-1 w-full"
                                        wire:model.live="invoices.{{ $id }}.check"
                                        :id="'check-number-' . $id"
                                    />
                                </x-bit.input.group>
                                <x-bit.input.group :for="'amount-' . $id" label="Amount">
                                    <x-bit.input.text
                                        leading-add-on="$"
                                        wire:model.live="invoices.{{ $id }}.amount"
                                        :id="'amount-' . $id"
                                    />
                                </x-bit.input.group>
                            </div>
            ...
            <x-slot name="footer">
                <x-bit.button.round.secondary wire:click="$set('showInvoiceModal', false)">
                    Cancel
                </x-bit.button.round.secondary>
