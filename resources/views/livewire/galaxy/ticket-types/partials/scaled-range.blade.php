    ...
    <div class="col-span-2">
        ...
        <div class="flex items-end space-x-4">
            <x-bit.input.group for="generate-prices-name" label="Name">
                <x-bit.input.text id="generate-prices-name" class="w-full mt-1" type="text" name="cost" wire:model.live="prices.0.name" />
            </x-bit.input.group>
            <x-bit.input.group for="generate-prices-min" label="Min">
                <x-bit.input.currency id="generate-prices-min" class="w-full mt-1" type="text" name="min" wire:model.live="prices.0.minInDollars" />
            </x-bit.input.group>
            <x-bit.input.group for="generate-prices-max" label="Max">
                <x-bit.input.currency id="generate-prices-max" class="w-full mt-1" type="text" name="max" wire:model.live="prices.0.maxInDollars" />
            </x-bit.input.group>
            <x-bit.input.group for="generate-prices-step" label="Step">
                <x-bit.input.text id="generate-prices-step" class="w-full mt-1" type="number" name="step" wire:model.live="prices.0.step" />
            </x-bit.input.group>
        </div>
    </div>
@endif
        ...
        <div class="space-y-4">
            ...
            <div wire:key="$index" class="flex items-end space-x-4">
                <x-bit.input.group :for="$index.'-prices-name'" label="Name">
                    <x-bit.input.text :id="$index.'-prices-name'" class="w-full mt-1" type="text" name="cost" wire:model.live="prices.{{ $index }}.name" />
                </x-bit.input.group>
                @if ($ticketType->structure === 'scaled-defined')
                <x-bit.input.group :for="$index.'-prices-description'" label="Description">
                    <x-bit.input.text :id="$index.'-prices-description'" class="w-full mt-1" type="text" name="cost" wire:model.live="prices.{{ $index }}.description" />
                </x-bit.input.group>
                @endif
                <x-bit.input.group :for="$index.'-prices-cost'" label="Cost">
                    <x-bit.input.currency :id="$index.'-prices-cost'" class="w-full mt-1" type="text" name="cost" wire:model.live="prices.{{ $index }}.costInDollars" />
                </x-bit.input.group>
                @if ($ticketType->structure === 'flat')
                <x-bit.input.group :for="$index.'-prices-start'" label="Availability Start">
                    <x-bit.input.date-time :id="$index.'-prices-start'" class="block w-full mt-1" name="start" wire:model.live="prices.{{ $index }}.formattedStart" />
                </x-bit.input.group>
                <x-bit.input.group :for="$index.'-prices-end'" label="Availability End">
                    <x-bit.input.date-time :id="$index.'-prices-end'" class="block w-full mt-1" name="end" wire:model.live="prices.{{ $index }}.formattedEnd" />
                </x-bit.input.group>
                @endif
