...
@if ($ticketType->prices->count() === 0)
    <div class="col-span-2">
        ...
        <p class="mb-4 text-sm dark:text-gray-300">
            We need to create a price for each step in the range. Instead of making you add each price individually,
            Enterprise will do it for you when saving the ticket type.
        </p>
        <div class="flex items-end space-x-4">
            <x-bit.input.group for="generate-prices-name" label="Name">
                <x-bit.input.text
                    id="generate-prices-name"
                    class="mt-1 w-full"
                    type="text"
                    name="cost"
                    wire:model.live="prices.0.name"
                />
            </x-bit.input.group>
            <x-bit.input.group for="generate-prices-min" label="Min">
                <x-bit.input.currency
                    id="generate-prices-min"
                    class="mt-1 w-full"
                    type="text"
                    name="min"
                    wire:model.live="prices.0.minInDollars"
                />
            </x-bit.input.group>
            <x-bit.input.group for="generate-prices-max" label="Max">
                <x-bit.input.currency
                    id="generate-prices-max"
                    class="mt-1 w-full"
                    type="text"
                    name="max"
                    wire:model.live="prices.0.maxInDollars"
                />
            </x-bit.input.group>
            <x-bit.input.group for="generate-prices-step" label="Step">
                <x-bit.input.text
                    id="generate-prices-step"
                    class="mt-1 w-full"
                    type="number"
                    name="step"
                    wire:model.live="prices.0.step"
                />
            </x-bit.input.group>
        </div>
    </div>
                    ...
                    @if ($ticketType->structure === 'flat')
                        <x-bit.input.group :for="$index.'-prices-start'" label="Availability Start">
                            <x-bit.input.date-time
                                :id="$index.'-prices-start'"
                                class="mt-1 block w-full"
                                name="start"
                                wire:model.live="prices.{{ $index }}.formattedStart"
                            />
                        </x-bit.input.group>
                        <x-bit.input.group :for="$index.'-prices-end'" label="Availability End">
                            <x-bit.input.date-time
                                ...
                                class="mt-1 block w-full"
                                name="end"
                                wire:model.live="prices.{{ $index }}.formattedEnd"
                            />
                        </x-bit.input.group>
                    @endif
