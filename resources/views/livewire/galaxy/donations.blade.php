                            ...
                            @unless ($selectAll)
                                <div>
                                    <span>
                                        ...
                                        donations, do you want to select all
                                        <strong>{{ number_format($donations->total()) }}</strong>
                                        ?
                                    </span>
                ...
                @forelse ($donations as $donation)
                    <x-bit.table.row wire:key="row-{{ $donation->id }}">
                        <x-bit.table.cell class="pr-0">
                            <x-form.checkbox
                                id="select-donation-{{ $donation->id }}"
                        ...
                        @if ($this->user === null)
                            <x-bit.table.cell>
                                <a href="{{ route('galaxy.users.show', $donation->user) }}" class="hover:underline">
                                    {{ $donation->user->name }}
                                </a>
                            </x-bit.table.cell>
                        @endif
                    ...
                    <x-bit.table.row>
                        <x-bit.table.cell colspan="11">
                            <div class="flex items-center justify-center space-x-2">
                                <x-heroicon-o-gift class="h-8 w-8 text-gray-400" />
