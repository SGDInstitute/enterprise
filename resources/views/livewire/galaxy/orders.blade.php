<div>
    <div class="mt-5 flex-col space-y-4">
        <div class="md:flex md:justify-between">
            <div class="flex flex-col space-y-4 md:flex-row md:items-end md:space-x-4">
            ...
            <x-slot name="body">
                ...
                @forelse ($orders as $order)
                    <x-bit.table.row wire:key="row-{{ $order->id }}">
                        ...
                        @if ($this->user === null)
                            <x-bit.table.cell>
                                <a href="{{ route('galaxy.users.show', $order->user) }}" class="hover:underline">
                                    {{ $order->user->name }}
                                </a>
                            </x-bit.table.cell>
                        @endif
                        ...
                        @if ($this->event === null)
                            <x-bit.table.cell>
                                <a href="{{ route('galaxy.events.show', $order->event) }}" class="hover:underline">
                                    {{ $order->event->name }}
                                </a>
                            </x-bit.table.cell>
                        @endif
                ...
                <x-slot name="content">
                    <div class="space-y-2">
                        @foreach ($editingOrder->tickets as $ticket)
                            <div class="flex items-center space-x-2 rounded border border-gray-900 px-3 py-2">
                                <x-bit.input.checkbox wire:model.live="editingTickets" value="{{ $ticket->id }}" />
                                <span class="dark:text-gray-200">
                                    {{ $ticket->user_id ? $ticket->user->name : 'Not filled' }}
                                </span>
                            </div>
                        @endforeach
                        ...
                        @if (count($editingTickets) > 0)
                            <div class="flex items-center justify-between p-4">
                                <span class="text-lg dark:text-gray-200">
                                    {{ count($editingTickets) }} Tickets to Refund
                                </span>
                                <span class="text-lg dark:text-gray-200">
                                    Amount to Refund: ${{ $editingTicketsAmount / 100 }}
                                </span>
                            </div>
                        @endif
                    </div>
