<div class="mt-12 space-y-12">
    ...
    <x-bit.modal.dialog wire:model.live="showPreviousOrders" max-width="6xl">
                ...
                <x-slot name="body">
                    @forelse ($previousOrders as $order)
                        <x-bit.table.row wire:key="row-{{ $order->id }}">
                            <x-bit.table.cell>
                                {{ $order->created_at->timezone($event->timezone)->format('M, d Y') }}
                            </x-bit.table.cell>
                            ...
                            <x-bit.table.cell colspan="9">
                                <div class="flex items-center justify-center space-x-2">
                                    ...
                                    <span class="py-8 text-xl font-medium text-gray-500 dark:text-gray-400">
                                        No existing reservations or orders found...
                                    </span>
                                </div>
                            </x-bit.table.cell>
                        </x-bit.table.row>
        ...
        <x-slot name="footer">
            <x-bit.button.flat.secondary wire:click="$set('showPreviousOrders', false)">
                Start a new order
