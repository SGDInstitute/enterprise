  ...
  <div class="space-y-4">
    ...
    <div class="relative flex items-start">
      ...
      <div class="ml-3 leading-6">
        <label for="{{ $item['id'] }}" class="font-medium text-gray-900 dark:text-gray-200">{{ $item['label'] }}</label>
        @isset($item['description'])
        <p id="{{ $item['id'] }}-description" class="text-gray-500 dark:text-gray-400">{{ $item['description'] }}</p>
        @endif
      </div>
    </div>
    @endforeach
