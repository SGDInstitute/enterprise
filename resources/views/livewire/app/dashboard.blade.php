<div class="container mx-auto px-4 pb-12 pt-4 md:px-12 md:py-12">
    <div class="grid grid-cols-1 gap-8 md:grid-cols-4">
        <nav class="space-y-1" aria-label="Sidebar">
            @if (auth()->user()->has_invitations)
                <x-galaxy.nav-link
                    :href="route('app.dashboard', 'invitations')"
            ...
            <x-galaxy.nav-link
                :href="route('app.dashboard', 'orders-reservations')"
                icon="heroicon-o-calendar"
                :active="'orders-reservations' === $page"
            >
                Event Tickets
            </x-galaxy.nav-link>
            <x-galaxy.nav-link
                :href="route('app.dashboard', 'workshops')"
                icon="heroicon-o-light-bulb"
                :active="'workshops' === $page"
            >
                Workshop Submissions
            </x-galaxy.nav-link>
            <x-galaxy.nav-link
                :href="route('app.dashboard', 'donations')"
                icon="heroicon-o-gift"
                :active="'donations' === $page"
            >
                Donations
            </x-galaxy.nav-link>
            <x-galaxy.nav-link
